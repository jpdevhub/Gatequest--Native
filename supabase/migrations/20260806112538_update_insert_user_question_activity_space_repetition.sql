...
-- Box 1: 1 week -> 3 days
-- Box 2: 2 weeks -> 6 days
-- Box 3: 4 weeks -> 12 days
...
declare
    ...
    v_is_verified boolean; -- Added for verification check
...
begin
    -- get the user version_number from the first item in the batch
    ...
    loop
        ...
        ------------------------------------------------------------------
        -- 3. Resolve ACTIVE weekly revision set
        ------------------------------------------------------------------
        select id
        into v_set_id
        ...
        ------------------------------------------------------------------
        -- 6. USER INCORRECT QUEUE (Spaced Repetition)
        ------------------------------------------------------------------
        if v_revision_state in ('none', 'first') then
            select box
            into v_box
            ...
            if found then
                ...
                if v_revision_state = 'first' then
                    if v_is_correct then
                        ...
                        elsif v_box = 2 then v_new_box := 3;
                        elsif v_box = 3 then
                    ...
                    if v_new_box is not null then
                        ...
                        set box = v_new_box,
                            next_review_at = case
                                when v_new_box = 1 then now() + interval '3 days'
                                when v_new_box = 2 then now() + interval '6 days'
                                when v_new_box = 3 then now() + interval '12 days'
                                else next_review_at
                            end
                        where user_id = v_user_id and question_id = v_question_id;
                    end if;
                ...
                if not v_is_correct then
                    ...
                    values (v_user_id, v_question_id, 1, now());
                end if;
            end if;
        end if;

        ------------------------------------------------------------------
        -- 7. REVISION SET UPDATE (first attempt only)
        ------------------------------------------------------------------
        if v_revision_state = 'first' then
            ...
            select count(*) into v_total_questions from revision_set_questions where set_id = v_set_id;
            select count(*) into v_attempted_questions from revision_set_questions where set_id = v_set_id and is_correct is not null;
            select count(*) into v_correct_questions from revision_set_questions where set_id = v_set_id and is_correct = true;
            ...
            set total_questions = v_total_questions,
                ...
                accuracy = case
                    ...
                    then round((v_correct_questions::numeric / v_attempted_questions) * 100, 2)
                    else null
                end
            where id = v_set_id;
