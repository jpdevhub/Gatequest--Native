...
create or replace function get_weekly_set(p_branch_id text)
returns json
language plpgsql
security definer
...
begin
    ...
    perform update_status_of_weekly_set(v_set_id => (
        ...
        where generated_for = v_user_id and branch_id = p_branch_id and status in ('pending', 'started') and expires_at <= now()
        limit 1
    ));
    ...
    where generated_for = v_user_id and branch_id = p_branch_id and status in ('pending', 'started')
    limit 1;
