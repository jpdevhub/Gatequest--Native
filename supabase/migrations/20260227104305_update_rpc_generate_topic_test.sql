...
BEGIN
  ...
  scored_questions AS (
    ...
    -- JOIN Condition: Matches BOTH Subject and Topic securely via UUIDs
    INNER JOIN jsonb_to_recordset(p_filters) AS f(subject_id uuid, topic text)
      ON q.subject_id = f.subject_id AND q.topic = f.topic
    LEFT JOIN user_history uh ON q.id = uh.question_id
    LEFT JOIN revision_queue rq ON q.id = rq.question_id
    -- THE FIX: Replaced the dangerous NOT IN subquery with a safe LEFT JOIN check
    WHERE (p_already_attempted_questions = TRUE OR uh.question_id IS NULL)
  ),
